...
def compile_html(documents: List[Any], brand: Dict[str, Any], pricing: Dict[str, Any]) -> str:
    ...
    for idx, doc in enumerate(documents):
        if isinstance(doc, str):
            ...
            normalised_sections.append(
                {
                    "title": doc.get("title", f"Section {idx + 1}"),
                    "html": doc.get("html", ""),
                }
            )
    ...
    if pricing and pricing.get("ok"):
        ...
        header_cols = "".join(f"<th>{c}</th>" for c in cols)
        rows_html = []
        for item in items:
            raw = item.get("raw", {})
            cells = []
        ...
        <div class=\"section\">
          ...
          <div class=\"pricing-total\">
            Grand Total: {currency} {grand:,} <br/>
            Grand Total (with mark‑up): {currency} {grand_mu:,}
          </div>
        </div>
        """
    ...
    <head>
      <meta charset=\"utf-8\">
      <title>Tender Proposal – {brand_name}</title>
      <style>
      {BASE_CSS}
